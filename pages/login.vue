...
const tryLogin = form.handleSubmit(async (values) => {
  pb.collection('users').authWithPassword(values.email, values.password).catch((r) => {
    ...
    <Card class="w-[512px]">
      ...
      <CardContent class="space-y-4">
              ...
              <FormControl>
                <div class="flex gap-2">
                  ...
                  <Button type="button" @click="passVis = !passVis" size="icon" variant="outline">
                    <Icon :name="passVis ? 'mdi:eye-off' : 'mdi:eye'" />
                  </Button>
                </div>
              </FormControl>
              <FormMessage />
        ...
        <Button class="w-full" variant="outline" type="button" @click="discordLogin()">
          <Icon class="text-lg" name="ic:round-discord" /> Continue with Discord
        </Button>
      </CardContent>
    </Card>
  </div>
